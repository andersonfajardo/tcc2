            ...
            <main class="main-content">
                ...
                <section class="action-plans">
                    ...
                    <div class="action-plan-form">
                        ...
                        <form @submit.prevent="addPlan" class="form-inline">
                            <input
                                v-model="newPlan.what"
                                type="text"
                                placeholder="O que?"
                                required
                            />
                            ...
                            />
                            <input
                                v-model="newPlan.when"
                                type="date"
                                placeholder="Quando?"
                                required
                            />
                            <input
                                v-model="newPlan.where"
                                type="text"
                                placeholder="Onde?"
                                required
                            />
                        ...
                        <select v-model="newPlan.dashboardType" id="dashboardType">
                          ...
                          <option value="Linha">Gráfico de Linha</option>
                                ...
                                <label for="id_indicator"
                                    >Selecione o Indicador:</label
                                >
                                <select
                                    id="id_indicator"
                                    @change="
                                        (e) =>
                                            (newPlan.id_indicator =
                                                e.target.value)
                                    "
                                >
                                    <option
                                        v-for="indicator in this.indicators"
                                        :value="indicator.id"
                                        :key="indicator.id"
                                    >
                                        {{ indicator.kpititle }}
    ...
    methods: {
        ...
        async addPlan() {
            ...
            try {
                ...
                const postdata = { ...this.newPlan }; // Sempre visível no dashboard
                postdata.postdata;
                ...
                if (this.isEditing) {
                    response = await axios.put(
                        '/actionplan/' + this.indicatorId,
                        postdata,
                    );
                } else {
                    response = await axios.post('/actionplan', postdata);
                }
